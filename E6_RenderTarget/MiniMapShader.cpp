...
void MiniMapShader::initShader(const wchar_t* vsFilename, const wchar_t* psFilename)
{
	...
	positionBufferDesc.ByteWidth = sizeof(PlayerPositionType);
...
void MiniMapShader::setShaderParameters(ID3D11DeviceContext* deviceContext,
	...
	Light* light)
{
	...
	PlayerPositionType* positionPtr;
	deviceContext->Map(positionBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	positionPtr = (PlayerPositionType*)mappedResource.pData;
	positionPtr->position = playerPos;
	//positionPtr->padding = 0.0f;
	deviceContext->Unmap(positionBuffer, 0);
	deviceContext->VSSetConstantBuffers(1, 1, &positionBuffer);
